void CurrentNorms(){
  ...
  vars = {"Nshw","Mult","Vy","Vx","Vz","Lmu","muCosTheta","muPhi","TrunMeandQdx","OpenAngle","ConvDist","Egamma","Mass","EgammaCorr","MassCorr"};
  ...
  for(int i = 0; i < sel.size(); i++){
    OnFile[i]  = 
      new TFile(Form("output_onbeam_%s.root",sel[i].c_str()));
    OffFile[i] = 
      new TFile(Form("output_offbeam_%s.root",sel[i].c_str()));
    MCFile[i]  = 
      new TFile(Form("output_mc_%s.root",sel[i].c_str()));      
  }
  ...
  for(int i = 0; i < sel.size(); i++){
    for(int j = 0; j < vars.size(); j++){
      ...
      for(int k = 0; k < bkgds.size(); k++){
	leg->AddEntry(MC[i][j][k], Form("%s,\n f : %4.2f",bkgds[k].c_str(),bkgd_int[i][j][k]/integral[i][j]), "f");
      }
