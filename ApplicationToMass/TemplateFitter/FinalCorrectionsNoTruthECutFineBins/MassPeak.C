...
void MassPeak(){
  ...
  for(int i = 0; i < sel.size(); i++){
    OnFile[i]  = 
      new TFile(Form("../../TemplateMaker/FinalCorrectionsNoTruthECutFineBins/output_onbeam_%s.root",sel[i].c_str()));
    OffFile[i] = 
      new TFile(Form("../../TemplateMaker/FinalCorrectionsNoTruthECutFineBins/output_offbeam_%s.root",sel[i].c_str()));
    MCFile[i]  = 
      new TFile(Form("../../TemplateMaker/FinalCorrectionsNoTruthECutFineBins/output_mc_%s.root",sel[i].c_str()));      
  }
  ...
  for(int i = 0; i < sel.size(); i++){
    for(int j = 0; j < vars.size(); j++){
      ...
      */
      ...
      TF1* fit = new TF1("fit",Fit,40,230,4);
