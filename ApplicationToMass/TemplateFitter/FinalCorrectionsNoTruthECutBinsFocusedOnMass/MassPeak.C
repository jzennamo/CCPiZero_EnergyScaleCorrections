...
void MassPeak(){
  ...
  for(int i = 0; i < sel.size(); i++){
    OnFile[i]  = 
      new TFile(Form("../../TemplateMaker/FinalCorrectionsNoTruthECutBinsFocusedOnMass/output_onbeam_%s.root",sel[i].c_str()));
    OffFile[i] = 
      new TFile(Form("../../TemplateMaker/FinalCorrectionsNoTruthECutBinsFocusedOnMass/output_offbeam_%s.root",sel[i].c_str()));
    MCFile[i]  = 
      new TFile(Form("../../TemplateMaker/FinalCorrectionsNoTruthECutBinsFocusedOnMass/output_mc_%s.root",sel[i].c_str()));      
  }
  ...
  for(int i = 0; i < sel.size(); i++){
	...
	On[i][j]->SetBinContent(b,On[i][j]->GetBinContent(b)/On[i][j]->GetBinWidth(b));
      ...
      for(int b = 1; b <= 11; b++){
	Off[i][j]->SetBinError(b,Off[i][j]->GetBinError(b)/Off[i][j]->GetBinWidth(b));
	Off[i][j]->SetBinContent(b,Off[i][j]->GetBinContent(b)/Off[i][j]->GetBinWidth(b));
      }
  ...
  for(int i = 0; i < sel.size(); i++){
    for(int j = 0; j < vars.size(); j++){
      ...
      On[i][j]->SetMaximum(On[i][j]->GetMaximum()*1.75);
      On[i][j]->SetMinimum(-0.1);
      On[i][j]->Draw("");            
      ...
      */
      ...
      TF1* fit = new TF1("fit",Fit,50,200,4);
      //fit->SetParLimits(0, 0, On[i][j]->GetMaximum()*1.3);
      fit->SetParameter(0, On[i][j]->GetMaximum());
      ...
      line.SetLineStyle(2);
      line.SetLineWidth(2);
      line.DrawLine(135,On[i][j]->GetMinimum(),135,On[i][j]->GetMaximum()*0.75);
      TLine line2;
      ...
      leg->AddEntry(On[i][j], "Data - Bkgds", "pl");   
      leg->AddEntry(fit, "ExpGaus Fit", "l");   
      ...
      Title2.DrawLatexNDC(0.5,0.85,"Fit Over Range M_{12} = [40,230]");

      Title2.DrawLatexNDC(0.5,0.78,Form("Fitted Peak : (%4.0f #pm %4.0f) MeV",
				       fit->GetParameter(1),
				       fit->GetParError(1)));
      Title2.DrawLatexNDC(0.5,0.73,Form("Fitted Width : (%4.0f #pm %4.0f) MeV",
