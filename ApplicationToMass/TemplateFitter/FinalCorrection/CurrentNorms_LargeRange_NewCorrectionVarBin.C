void CurrentNorms_LargeRange_NewCorrectionVarBin(){
  ...
  for(int i = 0; i < sel.size(); i++){
    OnFile[i]  = 
      new TFile(Form("../../TemplateMaker/FinalCorrections/output_onbeam_%s.root",sel[i].c_str()));
    OffFile[i] = 
      new TFile(Form("../../TemplateMaker/FinalCorrections/output_offbeam_%s.root",sel[i].c_str()));
    MCFile[i]  = 
      new TFile(Form("../../TemplateMaker/FinalCorrections/output_mc_%s.root",sel[i].c_str()));      
  }
  ...
  for(int i = 0; i < sel.size(); i++){
    for(int j = 0; j < vars.size(); j++){
      ...
      On[i][j]->SetMaximum(On[i][j]->GetMaximum()*1.5);
