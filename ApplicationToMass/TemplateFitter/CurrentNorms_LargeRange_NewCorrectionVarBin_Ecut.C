void CurrentNorms_LargeRange_NewCorrectionVarBin_Ecut(){
  ...
  for(int i = 0; i < sel.size(); i++){
    OnFile[i]  = 
      new TFile(Form("../TemplateMaker/NewCorrectionVarWidthWithEcut/output_onbeam_%s.root",sel[i].c_str()));
    OffFile[i] = 
      new TFile(Form("../TemplateMaker/NewCorrectionVarWidthWithEcut/output_offbeam_%s.root",sel[i].c_str()));
    MCFile[i]  = 
      new TFile(Form("../TemplateMaker/NewCorrectionVarWidthWithEcut/output_mc_%s.root",sel[i].c_str()));      
  }
