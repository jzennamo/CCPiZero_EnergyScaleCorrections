...
void shower_tree::Loop()
{
  ...
  for(int i = 0; i < 12; i++){
    ...
    leg->AddEntry(RecoE[i], Form("E_{Reco} [%4.0f,%4.0f]",step*i,step+step*i), "l");
    leg->AddEntry(TrueE[i], "Perfect Clustered Energy", "l");
    leg->AddEntry(fit[i], Form("GaussExp Fit [%4.0f,%4.0f]",TruncMean(TrueE[i])*0.5,TruncMean(TrueE[i])*1.5), "l");
    ...
    Correction.push_back(TruncMean(TrueE[i],x)/RecoE[i]->GetMean());
