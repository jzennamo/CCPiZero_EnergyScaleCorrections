...
void shower_tree::Loop()
{
  ...
  for(int i = 0; i < RecoE.size(); i++){
    ...
    TrueE[i] = new TH1D(Form("true_%d",i), ";Fraction of Deposited Energy in Perfectly Clustered Shower;", bins[i], 0, 1.2);       
    maxR.push_back(0);
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
	...
	RecoE[i]->Fill(shr_perfect_clustering_E);
	TrueE[i]->Fill(shr_perfect_clustering_E/shr_trueE_detProf);
  ...
  TH2D* hist = new TH2D("hist",";Fraction of Deposited Energy in Perfectly Clustered Hits;",1,0.4,1,1,0,250);
  hist->Draw();
