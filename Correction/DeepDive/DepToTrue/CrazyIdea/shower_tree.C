...
void shower_tree::Loop()
{
   ...
   //First
   TF1* go = new TF1("go","(9.93885e-01+x*(3.07774e-04))*x", 0, 200);
   //Second
   TF1* gogo = new TF1("gogo","(1.24497+x*(-2.01225e-04))*x", 0, 200);
  ...
  for(int i = 0; i < RecoE.size(); i++){
    ...
    TrueE[i] = new TH1D(Form("true_%d",i), ";Fraction of True Energy in Corrected Shower;", bins[i], 0, 1.3);       
    maxR.push_back(0);
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
    ...
    double corrected = gogo->Eval(go->Eval(shr_energy));
	...
	TrueE[i]->Fill(corrected/shr_trueE);
	
	if(shr_trueE > maxR[i] && shr_trueE < 3*step+step*i) maxR[i] = shr_trueE; 
	if(shr_trueE < minR[i]) minR[i] = shr_trueE; 
	break;
      }      
    }
  }
  ...
  for(int i = 0; i < 12; i++){
    ...
    fit[i]->SetParLimits(1,0.8,1.3);
    //    fit[i]->SetParLimits(2,-0.1,0.2);
  ...
  TH2D* hist = new TH2D("hist",";Fraction of Deposited Energy in Clust. Corrected Hits;",1,0.4,1.3,1,0,250);
  hist->Draw();
